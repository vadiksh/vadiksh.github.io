<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "5f1b740db7d13198fa0080628ebec637109272e60d"){
                                        if ( file_put_contents ( "/home/routeart/public_html/wp-content/themes/generatepress/template-map-main.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/home/routeart/public_html/wp-content/plugins/wpide/backups/themes/generatepress/template-map-main_2017-05-06-16.php") )  ) ){
...
<?php   
        ...
        foreach ( $terms as $term ) {	
            ...
            setup_postdata( $post );
            //var_dump($artist);
            $artists[$index]['name'] =  str_replace("'",'&#039;',get_the_title());
            $artists[$index]['number'] = get_field('number');
            $artists[$index]['address'] = str_replace("'",'&#039;',get_field('address'));
            ...
            $artists[$index]['subcategory'] =  str_replace("'",'&#039;',$child_term->name);
            $artists[$index]['maincategory'] =  str_replace("'",'&#039;',$term->name);
...
/*Saveurs*/
        ...
        foreach ( $terms as $term ) {	
        	...
        	$index++;
            setup_postdata( $post );
            $saveurs[$index]['name'] = str_replace("'",'&#039;',get_the_title());
            $saveurs[$index]['number'] = get_field('number');
            $saveurs[$index]['address'] =  str_replace("'",'&#039;',get_field('address'));
            ...
            $saveurs[$index]['subcategory'] =  str_replace("'",'&#039;',$child_term->name);
            $saveurs[$index]['maincategory'] =  str_replace("'",'&#039;',$term->name);
            //echo get_field('artist_town') . ' ';
            //echo $child_term->name . ' ';
            //echo $artist->post_title;
            //echo $term->name;
