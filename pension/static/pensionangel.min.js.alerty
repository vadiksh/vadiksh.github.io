alert('pensionangel.min.js was loaded!');
/*!
...
alert('Highcharts without errors');
...
alert('notifications without errors');
(function(){
 ...
 alert('Helper class without errors');
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  window.Store = {
      // New API call
    compare: function(){
      return $.get(apiUrl + '/compare');
    },
    
      // New API call
    compareAll: function(){
      return $.get(apiUrl + '/compare_all');
    },
...
*/    
    performance: function(){
      return setHeaders(apiUrl + '/performance');
    },

    improve: function(){
      return setHeaders(apiUrl + '/improve');
    },
    ...
    assessment: function(){
      ...
      return setHeaders(apiUrl + '/assessment');
    },

    riskAdvice: function(){
      return setHeaders(apiUrl + '/risk_advice');
    },

    showTrend: function(){
      return $.get(apiUrl + '/show_trend');
    },

    showTrends: function(){
      return $.get(apiUrl + '/show_trends');
    },
...
// owned categories if they are logged in
    compareBestDate: function(){
        return $.get(apiUrl + '/compare_best_date/<date variable?>');
    },

// Provide date for which to compare all fund categories - and user-
// owned categories if they are logged in
    compareAllDate: function(){
        return $.get(apiUrl + '/compare_all_date/<date variable?>');
    },
...
// and user-owned categories if they are logged in
    compareSelected: function(){
        return $.get(apiUrl + '/compare_selected/<fund category variable?>');
    },
...
// categories if they are logged in
    compareAllSelected: function(){
        return $.get(apiUrl + '/compare_all_selected/<fund category variable?>');
    },
...
// Otherwise, just show data for the selected fund category
    compareUser: function(){
        return $.get(apiUrl + '/compare_user/<fund category variable?>');
    },
...
// Get list of all user-owned funds and their Pension Angel categories
    myCategories: function(){
        return $.get(apiUrl + '/my_categories');
    },

    login: function(payload){
      if (!payload || !payload.email || !payload.password) return;
      
      return $.post(apiUrl + '/login', payload).done(function(data){
        if (data.jwt){
          data.data.jwt = jwt;
    ...
    logout: function(){
      if (!jwt) return;
      return $.get(apiUrl + '/logout').done(function(data){
        eraseCookie('jwt');
        jwt = undefined;
      })
    },
    ...
    register: function(payload){
      return $.post(apiUrl + '/register', payload)
      .then(function(data){
    ...
    update: function(payload){
      if (!payload){
        return setHeaders(apiUrl + '/update');
      } else {
        return setHeaders('https://api.pensionangel.com:5000/api/v1.0' + '/update_submission', {
          method: 'POST',
          data: payload
        }).then(function(res){
          Notifications.notify({
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  if (window.location.pathname.search(/\/login([./]{1}|$)/) == -1 ) return;
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  if (window.location.pathname.search(/\/register([./]{1}|$)/) == -1 ) return;
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  if (window.location.pathname.search(/\/show_trend([./]{1}|$)/) == -1 ) return;
  alert('show_trend page');
...
alert('show trends before DOMContentLoaded');
document.addEventListener("DOMContentLoaded", function(event) {
  alert('DOM content is loaded');
  if (window.location.pathname.search(/\/show_trends([./]{1}|$)/) == -1 ) return;
  
  alert('show_trends page');
   
  Store.showTrends().then(function(data){
    var derived = Helpers.derive(data);
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  if (window.location.pathname.search(/\/update([./]{1}|$)/) == -1 ) return;

  Store.update().then(function(data){
    data = data.data;
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  if (window.location.pathname.search(/\/assessment([./]{1}|$)/) == -1 ) return;
  console.log('assessment page');

  Store.assessment().then(function(result){
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  if (window.location.pathname.search(/\/improve([./]{1}|$)/) == -1 ) return;
  console.log('improve page');
  $('.btn-primary').click(function(){
    Notifications.notify({
      type: 'info',
      text: 'This is a really big information message. So big that it should take almost all space.',
      header: 'Just random info'
    })
  })
  ...
  $('.btn-success').click(function(){
   ...
   Store.improve().then(function(result){
    ...
    Store.riskAdvice().then(function(result){
      $('#riskAdvice h1').text(result.data['Risk Level:']);
      $("#riskAdvice p").text(result.data['New Contributions:']);
      $('#riskAdvice p:last-child').text(result.data['Existing Investments:'])
    });
  })
});

document.addEventListener("DOMContentLoaded", function(event) {
  if (window.location.pathname.search(/\/performance([./]{1}|$)/) == -1 ) return;
  console.log('performance page');
    // A code to run since DOM is loaded and ready

   Store.performance().then(function(data){
...
document.addEventListener("DOMContentLoaded", function(event) {
  if (window.location.pathname.search(/\/risk_advice([./]{1}|$)/) == -1 ) return;
