 ...
 */
 $(window).on("load resize",function(e){
	...
	*/
	if($('.img-hover').length){
		$(".img-hover.beeween-hover-2 .mask-over").each(function(index, val) {
 ...
 */
 (function($) {
	...
	*/
	...
	if($('.beeween-data-owl').length){

		$( '.beeween-data-owl' ).each(function( index ) {
			var $this =$(this);
			$this.owlCarousel(
			{
				items:$this.data('beeween_items'),
				navigation:$this.data('beeween_navigation'),
				singleItem:$this.data('beeween_singleitem'),
				autoPlay:$this.data('beeween_autoplay'),
				itemsScaleUp:$this.data('beeween_itemsscaleup'),
				navigationText:['<i class="icon-owl-navigation-left"></i>','<i class="icon-owl-navigation-right"></i>'], 
				pagination:$this.data('beeween_pagination'),
				paginationNumbers:$this.data('beeween_paginationnumbers'),
				autoHeight:$this.data('beeween_autoheight'),
				mouseDrag:$this.data('beeween_mousedrag'),
				transitionStyle:$this.data('beeween_transitionstyle'),
				responsive:true,
				lazyLoad : true,
				addClassActive:true,
				itemsDesktop :[1450,4],
				afterInit: function(){
					...
					$('.active .caption-content-position', $this).children().each(function(index, val) {
						$(this).addClass('animated '+$(this).data('beeween_animation'));
					});
				},
				beforeMove:  function(){
					$('.caption-content-position').children().each(function(index, val) {
						$(this).removeClass('animated '+$(this).data('beeween_animation'));
					});
				},
				afterMove:  function(){
					$('.active .caption-content-position', $this).children().each(function(index, val) {
	...
	*/
	if($('.activate-appear-animation').length){
		...
		$('.reloadAnim').click(function (e) {
			$(this).parent().parent().find('img[data-beeweenanim]').attr('class', '').addClass('img-responsive');
			beeweenAnimAppear();
			e.preventDefault();
		});
	}
 	...
 	if(!$('.swiper-preloader').hasClass('end-loading') && slideNumber != prevSlideNumber){
 		...
 		setTimeout(function() {
 			...
 			.done(function(html) {
 				...
 				$('.beeween-Ajax-loader').append('<article class="section-fw animated fadeInUp">'+html+'</article>');
 				$('.swiper-preloader').removeClass('visible');
 				slideNumber++;
 			})
 			.fail(function() {
 			...
 			if($this.parent().parent().is(jQuery(':gt(1)', $mainMenu))){
 				...
 				if($subMenu.css('display') == 'none'){
 					...
 					if($('.header-6').length || $('.header-7').length){
 						var $displayState = ($subMenu.hasClass('beeween-mega-menu'))?'table':'block';
 ...
 * @return {[type]} [description]
 */
 function beeweenAnimAppear(){
 	$("[data-beeweenanim]").each(function() {
 		...
 		if($(window).width() > 1024 && Modernizr.csstransitions) {
 			...
 			$this.appear(function() {
 				...
 				var delay = ($this.data("beeweendelay") ? $this.data("beeweendelay") : 1);
 				if(delay > 1) $this.css("animation-delay", delay + "ms");

 				$this.addClass("animated");
 				$this.addClass($this.data("beeweenanim"));
 ...
 */
 function toTop(mobile){
 	...
 	if(mobile == false){
 		...
 		if(!$('#beeweenToTop').length)
 			$('body').append('<a href="#" id="beeween-to-top"><i class="icon-to-top"></i></a>');

 		$(window).scroll(function () {
 			if ($(this).scrollTop() > 100) {
 				$('#beeween-to-top').slideDown('fast');
 			} else {
 				$('#beeween-to-top').slideUp('fast');
 			}
 		});

 		$('#beeween-to-top').click(function (e) {
 			e.preventDefault();
 			$("html, body").animate({
 				scrollTop: 0
 			}, 800, 'easeInOutCirc');
 		...
 		if($('#beeween-to-top').length)
 			$('#beeween-to-top').remove();
